...
visualize_age_standards_funct <- function(model_name, model, phonemes, phoneme_group_str, reference_col_str, posterior_samples) {
  ...
  filename <-  paste0("phoneme_numscore_mode_AAPS", ".RData") 
  ...
  ages <- seq(30, 120, by = 1) #coherent with the range of raw data
  newdata <- expand.grid(
  ...
  #Load AAPS filtered data for plot overlay
  
  # folder_path <- Paths$processed_data_dir
  # filename <- "df_final_AAPS.RData"
  # df_final_AAPS_path <- file.path(folder_path,filename)
  # df_final_AAPS_data <- load_from_rdata(df_final_AAPS_path,"df_final")
  # print(df_final_AAPS_data)
  # 
  # filename <- "phoneme_levels_AAPS.RData"
  ...
  age_plot <- ggplot(plot_data, aes(x = (age_months) / 12, y = q50, color = expected_phoneme)) +
    ...
    geom_point(
      ...
      aes(x = (age_months) / 12, y = mean_prob_jitter, color = expected_phoneme),
      inherit.aes = FALSE,
      alpha = 0.5,
      size = 1.5,
      #color ="darkred"
