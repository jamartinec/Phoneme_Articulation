...
plot_base_posterior <- function(plot_data, df_points, y_label = "Response") {
  ...
  ggplot(plot_data, aes(x = age_months/12, y = q50, color = expected_phoneme)) +
    geom_line(aes(linetype = "Posterior median"), linewidth = 1.1) +
    geom_ribbon(aes(ymin = q025, ymax = q975, fill = expected_phoneme),
                alpha = 0.2, color = NA) +
    geom_ribbon(aes(ymin = q25, ymax = q75, fill = expected_phoneme),
                alpha = 0.4, color = NA) +
    geom_point(
      data = df_points,
    ...
    scale_linetype_manual(values = c("Posterior median" = "solid"))
}


add_xq_overlay <- function(p, xq_df, label, color = "black",
                           extend_to_max = TRUE,
                           annotate = FALSE,
                           plot_data = NULL) {
  
  if (is.null(xq_df)) return(p)
  
  df <- xq_df %>% arrange(age_months)
  
  if (extend_to_max && !is.null(plot_data)) {
    max_age <- max(plot_data$age_months, na.rm = TRUE)
    last <- df %>% slice_tail(n = 1) %>% mutate(age_months = max_age)
    df <- bind_rows(df, last)
  }
  
  df <- df %>% mutate(age_years = age_months / 12)
  
  p <- p +
    geom_line(data = df,
              aes(x = age_years, y = x_q, linetype = label),
  ...
  if (annotate) {
    df <- df %>% mutate(coord_lab = sprintf("(%.1f, %.2f)", age_years, x_q))
    
    p <- p + ggrepel::geom_text_repel(
      data = df,
      aes(x = age_years, y = x_q, label = coord_lab),
      inherit.aes = FALSE, size = 3, color = color
    )
  }
...
# Create a function which include the mean and two bands (+2,-2) and return the plotted function.

plot_cutting_points<-function(plot_data, df_points, y_label, xq_predictive, m0, m2, p2, 
...
p <- add_xq_overlay(p, xq_predictive,           label1,     color=color1)
p <- add_xq_overlay(p, crow_tables$m0,          label2,     color=color2)
p <- add_xq_overlay(p, crow_tables$m2,          label3,     color=color3)
p <- add_xq_overlay(p, crow_tables$p2,          label4,     color=color4)
