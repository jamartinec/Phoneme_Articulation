...
save_all_credible_intervals_and_plot_forest <- function(
  ...
  get_credible_intervals <- function(category, levels, data_dir) {
    ...
    #load(file.path(data_dir,"model9", paste0("model_", group_str, ".RData")))
    model <- readRDS(file.path(data_dir,"model9", paste0("model_", group_str, ".rds")))
    ...
    rename_asymptote <- function(x) str_remove(x, "b_asymptote_expected_phoneme")
    ...
    asymptote_ci <- extract_parameter(
      posterior_samples, "b_asymptote",reference,
      rename_asymptote, plogis, "Asymptote"
    )
    
    bind_rows(alpha_ci, beta_ci, asymptote_ci) %>%
...
save_all_credible_intervals_and_plot_forest(
  ...
  output_dir = file.path(Paths$output_bayesian_dir,"model9")#"./output/bayesian_model"
)